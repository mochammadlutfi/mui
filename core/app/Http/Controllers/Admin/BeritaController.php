...
class BeritaController extends Controller
{
                ...
                ->addColumn('action', function($row){
                    ...
                    $btn = '<center><div class="btn-group" role="group">
                            ...
                            <div class="dropdown-menu" aria-labelledby="btnGroupVerticalDrop1" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 34px, 0px);">
                                ...
                                <a class="dropdown-item" href="'. route('admin.berita.komentar', $row->slug) .'">
                                    <i class="si si-bubbles mr-5"></i>Komentar
                                </a>
                            </div>
                        </div></center>';
    ...
    public function komentar(Request $request)
    {
        if ($request->ajax()) {
            if(!empty($request->filter_status))
            {
                $data = Komentar::where('status', $request->filter_status)->latest()->get();
            }else{
                $data = Komentar::latest()->get();
            }
            return Datatables::of($data)
                ->addIndexColumn()
                    ...
                    $btn = '<center><div class="btn-group" role="group">
                            ...
                            <div class="dropdown-menu" aria-labelledby="btnGroupVerticalDrop1" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 34px, 0px);">
                                <a class="dropdown-item" href="javascript:void(0)" onClick="edit('.$row->id.')">
                                    <i class="si si-note mr-5"></i>Lihat Komentar
                                </a>
                                <a class="dropdown-item" href="javascript:void(0)" onClick="hapus('.$row->id.')">
                                    <i class="si si-trash mr-5"></i>Hapus Komentar
                                </a>
                            </div>
                        </div></center>';
        ...
        return view('backend.berita.komentar');

    }

    public function komenhapus($id)
    {
        $berita = Komentar::find($id);
        $hapus_db = Komentar::destroy($berita->id);
        if($hapus_db)
        {
            return response()->json([
                'fail' => false,
    ...
    public function komenedit($id){
        $album = Komentar::find($id);
        $res = Collect([
            'id' => $album->id,
    ...
    public function komenupdate(Request $request)
    {
